  ...
  final routerProvider = Provider<GoRouter>((ref) {
    ...
    return GoRouter(
      ...
      redirect: (context, state) {
        ...
        // Not logged in on a protected route → go to login
        if (!isLoggedIn && !authRoutes.contains(loc)) {
          return AppRoutes.login;
        }
        // Not logged in on splash (loading done) → go to login
        if (!isLoggedIn && loc == AppRoutes.splash) {
          return AppRoutes.login;
        }
        // Logged in on an auth/splash route → go to home
        if (isLoggedIn && authRoutes.contains(loc)) {
